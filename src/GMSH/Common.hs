...
{-# LANGUAGE GADTs #-}
module GMSH.Common(BuilderStateData(..), BuilderMonadData(..), NonOverLappedClosedPoints(), toNonOverlappingClosedPoints, newBuilderData,  GPointId(..), eval) where
{- |
...
definitions
closed:
A list in which the last point is the same as head.
...
If they are overlapping, each point would exist twice, except the 1st and last points.
...
Return
A closed and nonoverlapping [Pts.Point]
...
-}
toNonOverlappingClosedPoints :: [Pts.Point] -> Either String [Pts.Point]
toNonOverlappingClosedPoints [] = Left "toNonOverlappingClosedPoints has [] passed in. Length must be at least 3 for a well formed surface"
...
toNonOverlappingClosedPoints points =
  toNonOverlappingClosedPoints' (head points) (head points) points []
