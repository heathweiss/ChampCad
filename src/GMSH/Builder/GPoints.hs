...
buildGPointsList h extraMsg points = 
...
buildGPointsList' :: SIO.Handle -> [GP.NonOverLappedClosedPoints] -> [GST.GPointId] -> GBB.ExceptStackCornerPointsBuilder [GST.GPointId]
...
buildGPointsList' h ((GP.NonOverLappedClosedPoints' noc_point):noc_points) workingList = do
  state' <- SL.get
  let
...
buildGPointsList_h h extraMsg points = 
  buildGPointsListOrFail_h h extraMsg points 
...
buildGPointsListOrFail_h' h ((GP.NonOverLappedClosedPoints' noc_point):noc_points) workingList = do
  ...
  case maybe_gpoint of
    ...
    Nothing -> do
      ...
      buildGPointsListOrFail_h' h noc_points newWorkingList
